...
For example, if the domain name is `wchun.xyz`, the current TLD server (which is responsible for `.xyz`) will return the IP address of the authoritative nameserver for `wchun`.
